#!/usr/bin/env python3
"""Build gen_data.py from scratch using the runner + ch*.txt files."""
import re
from pathlib import Path

GEN = Path(r"C:\Mockaroo\gen_data.py")
SCRIPT_DIR = Path(r"C:\Mockaroo")

# The runner function (canonical, with literal em-dashes)
...
# Build new content: runner + all 19 chapters
content = RUNNER + "\n\n"

for n in range(2, 21):
    fn = SCRIPT_DIR / f"ch{n}.txt"
    if not fn.exists():
        print(f"WARNING: missing {fn}")
        continue
    text = fn.read_text(encoding='utf-8')
    # Strip all CH{n}_WORD markers
    text = re.sub(rf"CH{n}_[A-Z0-9]+\n", "", text).strip()
    content += text + "\n\n"

GEN.write_text(content, encoding='utf-8', newline='\n')
print(f"gen_data.py rebuilt: {len(content)} chars, {content.count(chr(10))} lines")
